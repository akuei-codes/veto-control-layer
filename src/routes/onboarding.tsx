...
export const Route = createFileRoute("/onboarding")({
  head: () => ({
    meta: [
      { title: "Set up your workspace — Veto" },
      { name: "description", content: "Create your Veto workspace and connect your first AI agent." },
    ],
  }),
...
const AGENT_TYPES = [
  "Autonomous dev agents (code, refactor, deploy)",
  "Customer support agents",
  "DevOps / SRE agents",
  "Data / analytics agents",
  "Internal workflow / RPA agents",
  "Other",
];
...
function Onboarding() {
  ...
  const steps = ["Workspace", "You", "Agents"];

  const finish = () => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("veto.onboarded", "1");
      window.localStorage.setItem(
        "veto.workspace",
        JSON.stringify({ company, role, agentType, createdAt: Date.now() }),
      );
    }
  ...
  const canAdvance = step === 0 ? company.trim().length > 1 : step === 1 ? !!role : !!agentType;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      <main className="relative max-w-xl mx-auto px-6 pt-20 pb-16">
        {/* Step indicator */}
          ...
          >
            {step === 0 && (
              <Section
                ...
                hint="Every Veto workspace is isolated. Your agents, policies and audit trail live here."
              >
                <Field
                  label="Company name"
                  value={company}
            ...
            {step === 1 && (
              <Section
                ...
                hint="We'll tailor defaults and onboarding to match how your team operates."
              >
            ...
            {step === 2 && (
              <Section
                eyebrow="YOUR AGENTS"
                title="What kind of agents are you building?"
                hint="We'll seed sensible default policies and risk thresholds for this domain."
              >
        ...
        <div className="flex items-center justify-between mt-12">
          <button
            ...
            disabled={step === 0}
            className="font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:hover:text-muted-foreground transition-colors"
          >
            ← Back
          </button>
          <button
            onClick={next}
            disabled={!canAdvance}
            className="font-mono text-[11px] uppercase tracking-[0.2em] px-5 py-2.5 bg-foreground text-background rounded-sm hover:bg-foreground/90 disabled:opacity-30 disabled:hover:bg-foreground transition-colors"
          >
